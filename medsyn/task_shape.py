...
class PatchShapeMaker(ABC):
    @abstractmethod
    def get_patch_mask(
        self, dim_bounds: List[Tuple[int, int]], img_dims: np.ndarray
    ) -> np.ndarray:
        """
...
class PerlinPatchMaker(PatchShapeMaker):
    ...
    def rand_perlin_2d_np(
        self, shape, res, fade=lambda t: 6 * t**5 - 15 * t**4 + 10 * t**3
    ):
    ...
    def get_patch_mask_and_intersect_fn(
        self, dim_bounds: List[Tuple[int, int]], img_dims: np.ndarray
    ) -> Tuple[
        npt.NDArray[bool],
        ...
        perlin_scalex = (
            ...
            ** (
                np.random.randint(
                    low=self.min_perlin_scale, high=self.perlin_scale, size=(1,)
                )[0]
            )
        )
        perlin_scaley = (
        ...
        while True:
            perlin_noise = self.rand_perlin_2d_np(
                (noise_size_x, noise_size_y), (perlin_scalex, perlin_scaley)
            )
...
class DeformedHypershapePatchMaker(PatchShapeMaker):
    def __init__(
        self,
        aligned_distance_to_edge_fn: Callable[
            [npt.NDArray[int], npt.NDArray[float], npt.NDArray[float]],
            npt.NDArray[float],
        ],
    ...
    @abstractmethod
    def within_aligned_shape(
        self, array_of_coords: npt.NDArray[float], shape_size: npt.NDArray[int]
    ) -> npt.NDArray[bool]:
        """
    ...
    def make_shape_intersect_function(
        self,
        inv_trans_matrix: npt.NDArray[float],
        trans_matrix: npt.NDArray[float],
        shape_size: npt.NDArray[int],
    ) -> Callable[[npt.NDArray[float], npt.NDArray[float]], npt.NDArray[float]]:
        return lambda orig, direction: trans_matrix @ self.aligned_distance_to_edge_fn(
            shape_size, inv_trans_matrix @ orig, inv_trans_matrix @ direction
        )
    ...
    def get_patch_mask_and_intersect_fn(
        ...
        trans_matrix = functools.reduce(
            lambda m, ds: accumulate_rotation(
                m, self.rng.uniform(-np.pi / 2, np.pi / 2), ds
            ),
        ...
        corner_coord_grid = np.array(
            np.meshgrid(*np.stack([shape_width, -shape_width], axis=-1), indexing="ij")
        )
        ...
        coord_ranges = [
            np.arange(lb, ub + 1) for lb, ub in zip(min_trans_coords, max_trans_coords)
        ]
        ...
        return np.reshape(
            inv_result_grid_f, final_grid_shape.astype(int)
...
def intersect_to_aligned_hyperrectangle_edge(
    hyperrectangle_shape: npt.NDArray[int],
    ...
    # Shape (2D, N)
    all_edge_distances = np.concatenate([max_dir / direction, min_dir / direction])
    ...
    assert not np.any(min_is_inf), (
        f"Some lines are outside bounding box:\n"
...
class DeformedHyperrectanglePatchMaker(DeformedHypershapePatchMaker):
    ...
    def within_aligned_shape(
        ...
        return np.all(-rect_width <= array_of_coords, axis=0) & np.all(
            array_of_coords <= rect_width, axis=0
        )


def intersect_aligned_hyperellipse_edge(
    hyperellipse_shape: npt.NDArray[int],
    origin: npt.NDArray[float],
    direction: npt.NDArray[float],
) -> npt.NDArray[float]:
    ...
    assert not np.any(det_is_negative), (
        f"Some lines never intersect ellipse:\n"
    ...
    assert not np.any(min_is_inf), (
        f"Some lines are outside ellipse:\n"
        f"ellipse shape: {hyperellipse_shape},"
        f"origins - {origin[min_is_inf]},\n"
        f"directions -  {direction[min_is_inf]}"
    )
...
class CombinedDeformedHypershapePatchMaker(PatchShapeMaker):
    ...
    def get_patch_mask(
        ...
        if mode == "rect":
            ...
            big_m, small_m = (
                (rect_mask, ellip_mask)
                if rect_size >= ellip_size
                else (ellip_mask, rect_mask)
            )
            ...
            big_m[
                tuple([slice(lb, ub) for lb, ub in zip(lower_coord, upper_coord)])
            ] |= small_m
