...
class BaseTask(ABC):
    def __init__(self, sample_labeller: Optional[AnomalyLabeller] = None, **all_kwargs):
        ...
        self.anomaly_shape_maker = EitherDeformedHypershapePatchMaker(
            nsa_sample_dimension
        )
        self.all_kwargs = all_kwargs

    def apply(
        self, sample: npt.NDArray[float], *args, **kwargs
        ...
        for i in range(2):
            ...
            curr_anomaly_mask, intersect_fn = (
                self.anomaly_shape_maker.get_patch_mask_and_intersect_fn(
                    dim_bounds, sample_shape
                )
            )

            # Choose anomaly location
            anomaly_corner = self.find_valid_anomaly_location(
                curr_anomaly_mask, sample_mask, sample_shape
            )

            # Apply self-supervised task

            aug_sample = self.augment_sample(
                aug_sample, sample_mask, anomaly_corner, curr_anomaly_mask, intersect_fn
            )

            anomaly_mask[get_patch_slices(anomaly_corner, curr_anomaly_mask.shape)] |= (
                curr_anomaly_mask
            )
    ...
    def find_valid_anomaly_location(
        self,
        ...
        while True:
            ...
            target_patch_obj_mask = sample_mask[
                get_patch_slices(anomaly_corner, curr_anomaly_mask.shape)
            ]
            if (
                np.sum(target_patch_obj_mask & curr_anomaly_mask)
                / np.sum(curr_anomaly_mask)
            ) >= 0.5:
                break
...
class BasePatchBlendingTask(BaseTask):
    ...
    def augment_sample(
        ...
        assert len(source_sample_shape) == num_dims, (
        ...
        trans_matrix = functools.reduce(
            lambda m, ds: accumulate_rotation(
                m, self.rng.uniform(-np.pi / 4, np.pi / 4), ds
            ),
        ...
        corner_coords = np.array(
            np.meshgrid(
                *np.stack([np.zeros(num_dims), target_anomaly_shape], axis=-1),
                indexing="ij",
            )
        ).reshape(num_dims, 2**num_dims)
        ...
        # Extract source
        source_orig = source_sample[
            get_patch_image_slices(source_corner, tuple(final_init_grid_shape))
        ]

        # Because we computed the backwards transformation we don't need to inverse the matrix
        source_to_blend = np.stack(
            [
                affine_transform(
                    chan,
                    trans_matrix,
        ...
        aug_sample = self.blend_images(
            sample, source_to_blend, anomaly_corner, anomaly_mask
        )

        sample_slices = get_patch_image_slices(
            anomaly_corner, tuple(anomaly_mask.shape)
        )
        sample_diff = np.mean(
            np.abs(sample[sample_slices] - aug_sample[sample_slices]), axis=0
        )
...
class BaseDeformationTask(BaseTask):
    ...
    def augment_sample(
        ...
        mapping = self.compute_mapping(
            sample, sample_mask, anomaly_corner, anomaly_mask, anomaly_intersect_fn
        )
        sample_slices = get_patch_slices(anomaly_corner, tuple(anomaly_mask.shape))

        for chan in range(num_channels):
            sample[chan][sample_slices] = ndimage.map_coordinates(
                sample[chan][sample_slices], mapping, mode="nearest"
            )
        return sample
...
class RadialDeformationTask(BaseDeformationTask):
    def __init__(
        ...
        deform_centre: Optional[npt.NDArray] = None,
        **kwargs,
    ):
    ...
    def get_deform_factor(
        self, def_centre: npt.NDArray[int], anomaly_mask: npt.NDArray[bool]
    ):
        return (
            self.deform_factor
            if self.deform_factor is not None
            else 2 ** self.rng.uniform(0.5, 2)
        )

    @abstractmethod
    def compute_new_distance(
        self, curr_distance: float, max_distance: float, factor: float
    ) -> float:
        """
    ...
    def compute_mapping(
        ...
        def_centre = (
            self.deform_centre
            if self.deform_centre is not None
            else poss_centre_coords[:, np.random.randint(poss_centre_coords.shape[1])]
        )

        assert anomaly_mask[tuple(def_centre.round().astype(int))], (
            f"Centre is not within anomaly: {def_centre}"
        )
        ...
        mapping = np.stack(
            np.meshgrid(
                *[np.arange(s, dtype=float) for s in anomaly_shape], indexing="ij"
            ),
            axis=0,
        )
        ...
        anomaly_coords = mapping[map_inner_slice][
            (slice(None), anomaly_mask[mask_inner_slice])
        ]
        ...
        mask_edge_intersections = (
            anomaly_intersect_fn(exp_def_centre - exp_mask_centre, all_coords_norm_dirs)
            + exp_mask_centre
        )
        ...
        new_coord_distances = self.compute_new_distance(
            all_coords_distance, mask_edge_distances, def_factor
        )
        # (D, N)
        new_coords = exp_def_centre + new_coord_distances * all_coords_norm_dirs

        mapping[map_inner_slice][(slice(None), anomaly_mask[mask_inner_slice])] = (
            new_coords
        )
...
class CutPastePatchBlender(BasePatchBlendingTask):
    def __init__(self, source_images: list, Labelber_std: float = 0.2, **kwargs):
        sample_labeller = FlippedGaussianLabeller(Labelber_std)
        source_images = [
            np.expand_dims(image, axis=0) if len(image.shape) == 2 else image
...
class SmoothIntensityChangeTask(BaseTask):
    ...
    def augment_sample(
        ...
        intensity_changes = (
            self.intensity_task_scale / 2
            + np.random.gamma(3, self.intensity_task_scale)
        ) * np.random.choice([1, -1], size=num_chans)

        intensity_change_map = smooth_dist_map * np.reshape(
            intensity_changes, [-1] + [1] * num_dims
        )
        ...
        sample[anomaly_patch_slices] = np.clip(
            new_patch,
...
class GaussIntensityChangeTask(BaseTask):
    ...
    def get_predefined_texture(
        ...
        random_sample = (random_sample >= 0.0).astype(
            float
        )  # int type can't do Gaussian filter

        random_sample = gaussian_filter(random_sample, sigma_b)

        random_sample = (random_sample - np.min(random_sample)) / (
    ...
    def create_texture(self, sizes):
        texture = self.get_predefined_texture(
            sizes,
    ...
    def augment_sample(
        ...
        anomaly_patch_slices = get_patch_image_slices(
            anomaly_corner, anomaly_mask_copy.shape
        )
        ...
        for cha in range(sample.shape[0]):
            sample[anomaly_patch_slices][cha] = (
                sample[anomaly_patch_slices][cha] * (1 - anomaly_mask_copy)
                + (
                    sample[anomaly_patch_slices][cha]
                    + texture[anomaly_patch_slices][cha] * geo_blur
                )
                * anomaly_mask_copy
            )
...
class SinkDeformationTask(RadialDeformationTask):
    ...
    def compute_new_distance(
        self,
        curr_distance: Union[float, npt.NDArray[float]],
        max_distance: Union[float, npt.NDArray[float]],
        factor: Union[float, npt.NDArray[float]],
    ) -> Union[float, npt.NDArray[float]]:
        return max_distance - (
            max_distance - curr_distance
        ) ** factor / max_distance ** (factor - 1)
