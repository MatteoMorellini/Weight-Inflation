...
class IntensityDiffLabeller(AnomalyLabeller, ABC):
    ...
    @abstractmethod
    def label_fn(
        self, x: Union[npt.NDArray[float], float]
    ...
    def label(
        ...
        if num_dims not in self.binary_structures:
            self.binary_structures[num_dims] = ndimage.generate_binary_structure(
                num_dims, 2
            )
        ...
        for anom_slice in ndimage.find_objects(ndimage.label(mask)[0]):
            anom_region_label = ndimage.grey_closing(
                scaled_diff[anom_slice], footprint=bin_structure
            )

            recon_seed = np.copy(anom_region_label)
            recon_seed[num_dims * (slice(1, -1),)] = anom_region_label.max()
            scaled_diff[anom_slice] = morphology.reconstruction(
                recon_seed,
                anom_region_label,
                method="erosion",
            )
